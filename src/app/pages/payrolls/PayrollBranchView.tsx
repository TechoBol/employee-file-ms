...
import type { PayrollByBranchResponse } from '@/rest-client/interface/response/PayrollResponse';
import type { ColumnDef } from '@tanstack/react-table';

interface PayrollBranchViewProps {
  data: PayrollByBranchResponse[];
  loading: boolean;
...
export function PayrollBranchView({
  data,
  loading,
  columns,
  deductionLabels,
  formatCurrency,
}: PayrollBranchViewProps) {
  if (loading) {
    return (
      <div className="text-center p-8">
        <RefreshCw className="h-8 w-8 animate-spin mx-auto mb-4 text-muted-foreground" />
        <p className="text-muted-foreground">Cargando nóminas agrupadas...</p>
      </div>
    );
  }
  ...
  if (!data || data.length === 0) {
    return (
      <div className="text-center p-8 border rounded-xl">
        <p className="text-muted-foreground">No se encontraron sucursales con nóminas</p>
      </div>
    );
  }
      ...
      {data.map((branch) => (
        <div key={branch.branchId} className="space-y-3">
          ...
          <div className="rounded-lg bg-card">
            <DataTable
              columns={columns}
              data={branch.PayrollEmployeeResponse || []}
              loading={false}
